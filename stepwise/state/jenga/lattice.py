...
`predicates()` binds the previous snapshot, and `collapse_detected` -- which is
about now, not before -- is the sole exception.
"""
...
class TowerLattice(WorldState):
    ...
    def before(self) -> Snapshot:
        """The reading a move is judged against; see the module docstring."""
        return self._previous

    def observe(self, occupancy: Sequence[Sequence[int]], t: float = 0.0) -> None:
        ...
        for layer in range(height):
            for slot in range(self.slots_per_layer):
                ...
                if was == now:
                    continue
                if was and not now:
                    removes.append(Event(t=t, kind="REMOVE", src=Slot(layer, slot)))
                else:
    ...
    def predicates(self):
        """Bind the rulebook's vocabulary to the pre-move reading."""
        snap = self._previous
        names = {
